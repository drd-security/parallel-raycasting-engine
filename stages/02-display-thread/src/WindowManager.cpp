...
unsigned int WindowManager::getKeysPressed() { return keysPressed.load(); }
...
void WindowManager::updateDisplay()
{
    ...
    doubleBuffer.copyBackBuffer(localCopy);
    std::copy(localCopy.begin(), localCopy.end(), imgBuffer);
    XPutImage(display, window, gc, img, 0, 0, 0, 0, width, height);
}
